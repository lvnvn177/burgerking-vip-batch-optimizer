...
public class CouponIntegrationTest {
    ...
    void createCouponTest() throws Exception {
        ...
        CouponRequest request = CouponRequest.builder()
            ...
            .couponType(CouponType.FIXED_AMOUNT) // CouponType.FIXED_AMOUNT로 수정
        ...
        Long newCouponId = objectMapper.readTree(responseContent).get("id").asLong();


        // DB에 정상적으로 저장되었는지 확인 (새로운 ID로 조회)
        Optional<Coupon> createdCouponOpt = couponRepository.findById(newCouponId);
        assertThat(createdCouponOpt).isPresent();
        assertThat(createdCouponOpt.get().getDescription()).isEqualTo("새로 생성된 통합 테스트 쿠폰입니다");
        
        // 재고도 함께 생성되었는지 확인
        Optional<CouponStock> stock = couponStockRepository.findByCouponId(newCouponId);
    ...
    void getCouponTest() throws Exception {
        // When & Then
        mockMvc.perform(get("/api/coupons/{couponId}", testCouponId)
                .contentType(MediaType.APPLICATION_JSON))
    ...
    void issueCouponSuccessTest() throws Exception {
        // Given
        CouponIssueRequest request = new CouponIssueRequest(TEST_USER_ID, testCouponId);
        ...
        mockMvc.perform(post("/api/coupons/issue")
                ...
                .andExpect(jsonPath("$.error").value("COUPON_ALREADY_ISSUED"));
    }
    ...
    void useCouponTest() throws Exception {
        // Given - 먼저 쿠폰을 발급받음
        CouponIssueRequest issueRequest = new CouponIssueRequest(TEST_USER_ID, testCouponId);
        mockMvc.perform(post("/api/coupons/issue")
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void getUserCouponsTest() throws Exception {
        ...
        mockMvc.perform(get("/api/coupons/user/{userId}", TEST_USER_ID)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].couponId").value(testCouponId))
