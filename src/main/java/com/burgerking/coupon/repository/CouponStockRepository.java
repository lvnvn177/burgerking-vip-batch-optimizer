...
public interface CouponStockRepository extends JpaRepository<CouponStock, Long> {
     ...
     */
    @Modifying
    @Query("UPDATE CouponStock cs SET cs.remainingQuantity = cs.remainingQuantity - 1 WHERE cs.coupon.id = :couponId AND cs.remainingQuantity > 0")
    int decreaseStockAtomic(@Param("couponId") Long couponId);
